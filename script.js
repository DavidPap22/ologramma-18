...
let bgSavedTime = 0;
let cameraFacingInterval;
const wait = ms => new Promise(r => setTimeout(r, ms));

// ===== ROTAZIONE SMOOTH =====
function asVec3(v){ return { x: v.x || v[0] || 0, y: v.y || v[1] || 0, z: v.z || v[2] || 0 }; }
function computeYawToTarget(item, target){ return Math.atan2(target.x - item.x, target.z - item.z) * (180/Math.PI); }
function setItemYawOnly(el, yaw){ let r = el.getAttribute('rotation') || {x:0, y:0, z:0}; r.y = yaw; el.setAttribute('rotation', r); }
function orientItemToCameraY(el){
    ...
    const itemPos = asVec3(posAttr);
    const targetYaw = computeYawToTarget(itemPos, { x: camWorldPos.x, y: camWorldPos.y, z: camWorldPos.z });
    const currentRotation = el.getAttribute('rotation') || {x:0,y:0,z:0};
    let currentYaw = parseFloat(currentRotation.y) || 0;
    const lerpFactor = 0.08; // modifica per velocità rotazione
    let newYaw = currentYaw + (targetYaw - currentYaw) * lerpFactor;
    setItemYawOnly(el, newYaw);
}
function startCameraFacingUpdater(active = true, freqMs = 50){
...
startBtn.addEventListener('click', async () => {
    ...
    startCameraFacingUpdater(true,50);
    ...
    const tid=setInterval(()=>{
        try{
            ...
            if(mesh && mesh.material && mesh.material.map){
                mesh.material.map.needsUpdate=true; mesh.material.needsUpdate=true;
            }
        } catch(e){}
...
function createSmoke(count=20){
    ...
    for(let i=0;i<count;i++){
        ...
        e.setAttribute('animation',`property: position; to: ${px.toFixed(3)} ${(py+0.6).toFixed(3)} ${pz.toFixed(3)}; dur:${dur}; dir:alternate; loop:true; easing:easeInOutSine`);
        root.appendChild(e);
    }
}
...
function setupInteractions(){
    ...
    qr.addEventListener('click', async ()=>{
        ...
        try { await holoVideo.play(); bgSavedTime = bgMusic.currentTime; bgMusic.pause(); }
        catch(e){ videoTapOverlay.style.display='flex'; }
    });

    tapToPlay && tapToPlay.addEventListener('click', async ()=>{
        videoTapOverlay.style.display='none';
        try { await holoVideo.play(); bgSavedTime = bgMusic.currentTime; bgMusic.pause(); }
        catch(e){ alert('Impossibile avviare il video'); }
    });
    ...
    itemIds.forEach(id=>{
        ...
        el.addEventListener('click', ()=>{
            ...
            if(linkMap[id]) window.open(linkMap[id],'_blank');
        });
    });
}
...
function preserveVideoAspect(){
    demoVideo.addEventListener('loadedmetadata', ()=>{
        ...
        const ar = w/h;
        demoVideo.setAttribute('scale',`${1*ar} 1 1`);
    });
}
