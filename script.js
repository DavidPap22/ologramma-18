// ---------- script.js completo (versione iOS-friendly, face-camera, velocità sfere aumentata) ----------
...
// Assicurati che il video abbia volume alto (massimo) quando possibile
...
const playingAudios = {}; // playingAudios[id] = Audio object or truthy
...
function distributeItemsCircle(radius = 2.0, height = 2.2) {
  ...
  itemIds.forEach((id, i) => {
    ...
    const dur = 800 + Math.random() * 600; // velocità aumentata (dur più corta)
...
function createParticles(count = 32) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const dur = 800 + Math.random() * 1000; // più veloce
...
function setupInteractions() {
  ...
  qr.addEventListener('click', async () => {
    ...
    if (isIOS) {
      // mostra overlay: user touch sul bottone => play() considerato gesto nativo
      videoTapOverlay.style.display = 'flex';
      return;
    }

    // non-iOS: prova a playare subito, fallback all'overlay se fallisce
    try {
  ...
  // Bottone Avvia Video (touch nativo)
  tapToPlay && tapToPlay.addEventListener('click', async () => {
    videoTapOverlay.style.display = 'none';
    try {
      holoVideo.volume = 1.0;
      await holoVideo.play(); // avviato da gesture nativa => iOS permette l'audio
      bgSavedTime = bgMusic.currentTime;
      bgMusic.pause();
    } catch (e) {
  ...
  replayLogo.addEventListener('click', async () => {
    ...
    if (/iPad|iPhone|iPod/.test(navigator.userAgent) && !window.MSStream) {
      // iOS: mostra overlay per far partire il video con gesto nativo
      videoTapOverlay.style.display = 'flex';
      return;
    }
  ...
  // WhatsApp: usa il canale fornito
  ...
  // item clicks (link first, then local audio)
  itemIds.forEach(id => {
    const el = document.getElementById(id); if (!el) return;
    el.addEventListener('click', () => {
      // link priorità (non interrompono bgMusic)
      if (linkMap[id]) {
        window.open(linkMap[id], '_blank');
        return;
      }
      // local audio: play solo se non in riproduzione
      if (audioMap[id]) {
...
// Nota: setupInteractions() viene registrata e chiamata al click di START nel flusso principale.
