...
// ===== CONFIGURAZIONE ORIENTAMENTO (tutti guardano la camera) =====
const orientationConfig = {};
itemIds.forEach(id => { orientationConfig[id] = { face: 'camera' }; });

// ====== HELPERS PER ORIENTAMENTO ======
const radToDeg = r => r * 180 / Math.PI;
function computeYawToTarget(itemPos, targetPos) {
  const dx = targetPos.x - itemPos.x;
  const dz = targetPos.z - itemPos.z;
  const yawRad = Math.atan2(dx, dz);
  return radToDeg(yawRad);
}
function setItemYawOnly(el, yawDeg) { el.removeAttribute('look-at'); el.setAttribute('rotation', `0 ${yawDeg.toFixed(2)} 0`); }
function asVec3(v) { if (!v) return null; if (typeof v === 'string') { const [x,y,z] = v.split(/\s+/).map(Number); return {x,y,z}; } return v; }
function orientItemToPositionY(el, targetPos) {
  const posAttr = el.getAttribute('position') || {x:0,y:0,z:0};
  const itemPos = asVec3(posAttr) || { x:0, y:0, z:0 };
  const t = asVec3(targetPos);
  if (!t) return;
  const yaw = computeYawToTarget(itemPos, t);
  setItemYawOnly(el, yaw);
}
function orientItemToCameraY(el) {
  const cameraEl = document.querySelector('#camera');
  if (!cameraEl || !cameraEl.object3D) return;
  const camWorldPos = new THREE.Vector3();
  cameraEl.object3D.getWorldPosition(camWorldPos);
  orientItemToPositionY(el, { x: camWorldPos.x, y: camWorldPos.y, z: camWorldPos.z });
}
let cameraFacingIds = [];
function applyOrientationConfig(cfg = {}) {
  cameraFacingIds = [];
  Object.keys(cfg).forEach(id => {
    const el = document.getElementById(id); if (!el) return;
    const c = cfg[id];
    if (c.face === 'camera') { orientItemToCameraY(el); cameraFacingIds.push(id); }
    else if (c.face && typeof c.face === 'object') orientItemToPositionY(el, c.face);
    else if (typeof c.yaw !== 'undefined') setItemYawOnly(el, c.yaw);
    else if (typeof c.rotation !== 'undefined') el.setAttribute('rotation', String(c.rotation));
  });
}
let cameraFacingInterval = null;
function startCameraFacingUpdater(active = true, freqMs = 200) {
  if (cameraFacingInterval) clearInterval(cameraFacingInterval);
  if (!active) return;
  cameraFacingInterval = setInterval(()=>{
    cameraFacingIds.forEach(id=>{
      const el = document.getElementById(id); if(!el)return; orientItemToCameraY(el);
    });
  }, freqMs);
}
...
startBtn.addEventListener('click',async()=>{
  ...
  applyOrientationConfig(orientationConfig);
  startCameraFacingUpdater(true,200);
